use eth_light_client_in_ckb_verification::{
    ...
    utilities::ssz::{
        ceil_depth, floor_depth, is_valid_merkle_branch, length_hash, verify_merkle_proof,
    },
};
...
macro_rules! create_test_for_verify {
    ($test_name:ident, $func_wrapper:ident) => {
        #[test]
        fn $test_name() {
            let d_vec = (0u8..=10)
...

            {
                ...
                for index in 0..d_vec_len {
                    ...
                    let result = $func_wrapper!(&root, leaf, &proof, generalized_index);
                    assert!(
                        result,
...
macro_rules! verify_merkle_proof_wrapper {
    ($root:expr, $leaf:expr, $proof:expr, $generalized_index:expr) => {
        verify_merkle_proof($root, $leaf, $proof, $generalized_index)
    };
}

macro_rules! is_valid_merkle_branch_wrapper {
    ($root:expr, $leaf:expr, $proof:expr, $generalized_index:expr) => {{
        let depth = helpers::floorlog2($generalized_index) as usize;
        let index = helpers::get_subtree_index($generalized_index);
        is_valid_merkle_branch($leaf, $proof, depth, index, $root)
    }};
}

create_test_for_verify!(test_verify_merkle_proof, verify_merkle_proof_wrapper);
create_test_for_verify!(test_is_valid_merkle_branch, is_valid_merkle_branch_wrapper);
